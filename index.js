...
io.on("connection", (socket) => {
    ...
    socket.on('call',({from,to})=>{
      ...
      io.to(to).emit('incoming-call',{from,to})
    })
    socket.on('call-ended',({to})=>{
      console.log('sending call end msg to ', to)
      io.to(to).emit('call-ended-from',{to})
    })
    socket.on('answer',({from,to,stream})=>{
      console.log('sending answer back to ', to)
      console.log('sending stream',stream)
      io.to(to).emit('call-answered',{from,remoteStream:stream})
    })
    socket.on('stream',({from,to,stream})=>{
      io.to(to).emit('stream',{from,stream})
    })
